...
import {
  ...
  FaCalendarAlt, FaHeart, FaChevronLeft
} from "react-icons/fa";
...
const DetailFactCard = ({ icon: Icon, title, value, colorClass, delay }) => (
  ...
  >
    <div className={`w-10 h-10 rounded-lg mb-3 flex items-center justify-center bg-base-200 transition-colors group-hover:bg-primary group-hover:text-primary-content ${colorClass}`}>
      <Icon className="text-lg" />
    </div>
    <p className="text-[10px] uppercase font-bold tracking-widest text-base-content-muted mb-1">{title}</p>
    <p className="font-semibold text-base text-base-content truncate" title={value}>
      {value}
    </p>
  </div>
);
  ...
  return (
    <div className="min-h-screen bg-base-200 py-12 px-4 sm:px-8 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto mb-10 flex items-center justify-between">
        <button onClick={() => navigate(-1)} className="group flex items-center gap-2 text-sm font-medium text-base-content-muted hover:text-primary transition-colors">
        ...
        <div className="flex items-center gap-2 px-3 py-1 bg-base-100 rounded-full shadow-sm">
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider text-base-content-muted">Live Status</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="sticky top-10">
            <div className="relative group rounded-3xl overflow-hidden shadow-2xl">
              <img src={photo} alt={bookTitle} className="w-full h-auto object-cover" />
            ...
            <div className="mt-8 bg-base-100 p-8 rounded-3xl shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-base-content-muted mb-1">Price</p>
                  <h3 className="text-3xl font-black text-base-content">${price}</h3>
                </div>
        ...
        <div className="lg:col-span-8 space-y-12">
          <section className="animate-fadeInUp">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-xs font-bold uppercase tracking-[0.2em] text-primary">{category}</span>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
            <DetailFactCard icon={FaStar} title="Rating" value={`${rating} / 5`} colorClass="text-yellow-500" delay={100} />
            <DetailFactCard icon={FaCheckCircle} title="Status" value={status} colorClass="text-success-main" delay={200} />
            <DetailFactCard icon={FaRulerVertical} title="Pages" value={page} colorClass="text-info" delay={300} />
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center p-5 bg-base-100 rounded-2xl shadow-sm gap-4">
              ...
              <div>
                <p className="text-[10px] font-bold text-base-content-muted uppercase tracking-widest">Published</p>
                <p className="font-semibold text-base-content">{publicationDate}</p>
              </div>
            </div>
            <div className="flex items-center p-5 bg-base-100 rounded-2xl shadow-sm gap-4">
              <FaBook className="text-xl text-primary/40" />
              <div>
                <p className="text-[10px] font-bold text-base-content-muted uppercase tracking-widest">ISBN</p>
