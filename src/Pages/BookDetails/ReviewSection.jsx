...
const ReviewSection = ({ bookId, userEmail, userName, refetchBook }) => {
    ...
    // --- QUERY 1: Check Review Eligibility ---
    const { data: eligibility = {}, isLoading: isLoadingEligibility } = useQuery({
        queryKey: ['reviewEligibility', bookId, userEmail],
        enabled: !!userEmail && !!bookId, // Only run if user is logged in
        queryFn: async () => {
            if (!userEmail) return { canReview: false };
            const res = await axios.get(`${API_BASE_URL}/user-can-review/${bookId}/${userEmail}`);
            return res.data;
        },
    });

    // --- QUERY 2: Fetch All Reviews ---
    const { data: reviews = [], isLoading: isLoadingReviews } = useQuery({
        queryKey: ['bookReviews', bookId],
        queryFn: async () => {
            const res = await axios.get(`${API_BASE_URL}/reviews/${bookId}`);
            return res.data;
        },
    });
    ...
    const reviewMutation = useMutation({
        mutationFn: (newReview) => axios.post(`${API_BASE_URL}/reviews`, newReview),
        onSuccess: (response) => {
